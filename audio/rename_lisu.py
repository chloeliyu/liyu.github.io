...
------------------------
用法：
  1. 把这个脚本文件放到 lisu 文件夹【外面】，跟 lisu 文件夹同级
     （或者直接把下面 FOLDER 改成 lisu 文件夹的完整路径）。
  2. 确保 lisu 文件夹里现在的 100 个录音文件名里带有数字编号
     （比如 1.m4a / 录音01.m4a / rec_1.m4a 都可以，只要数字能看出顺序）。
  3. 用命令行运行：  python rename_lisu.py
...
# ↓↓↓ 如果脚本没有和 lisu 文件夹放在一起，把这里改成文件夹的完整路径 ↓↓↓
FOLDER = Path(__file__).parent / "lisu"
